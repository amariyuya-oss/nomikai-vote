...
|------------|--------|-----------|------|--------|
| name | 飲み会名 | text | ✅ | 空 |
| event_date | 開催日 | date（input[type=date]） | ✅ | 空 |
| event_time | 開催時間 | select（11:00〜23:30、30分刻み） | ✅ | 空 |
| location | 開催場所 | text | - | 空 |
| nearest_station | 最寄駅 | text | - | 空 |

※ `event_date` と `event_time` を組み合わせてサーバーへ ISO8601 文字列として送信する。
...
| event_date | 必須 | 「開催日を入力してください」 |
| event_time | 必須・event_date と組み合わせた日時が現在時刻より未来 | 「開催時間を選択してください」／「開催日時は現在以降を指定してください」 |
